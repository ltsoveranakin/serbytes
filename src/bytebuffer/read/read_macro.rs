macro_rules! read_ref_ty {
    ($t: ty, $call: ident, $size: literal) => {
        ...
        pub fn $call(&mut self) -> crate::bytebuffer::BBReadResult<$t> {
            ...
            let bytes = crate::bytebuffer::WithParent::with_parent(crate::bytebuffer::ReadByteBufferRefMut::read_bytes(self, $size), stringify!($t))?;
...
macro_rules! read_owned_ty {
    ($t: ty, $call: ident) => {
        ...
        pub fn $call(&mut self) -> crate::bytebuffer::BBReadResult<$t> {
            crate::bytebuffer::ReadByteBufferRefMut::$call(
                &mut crate::bytebuffer::ReadByteBufferOwned::rbb_ref_mut(self), /* usize */
            )
        }
    };
}
